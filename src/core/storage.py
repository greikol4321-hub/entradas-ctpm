...
def _assert_https(url):
    if not url.startswith("https://"):
        raise ValueError("Supabase URL debe ser https")
    import ssl
...
def supabase_upload(bucket, fname, data_bytes, content_type="application/octet-stream"):
    ...
    try:
        ...
        _ctx = _assert_https(url)
        req = urllib.request.Request(url, data=data_bytes, method="POST", headers=_supabase_headers(content_type))
        try:
            with urllib.request.urlopen(req, timeout=10, context=_ctx) as resp:  # nosec B310 - URL SUPABASE env, https validado
            ...
            if e.code in (409, 400):
                ...
                with urllib.request.urlopen(req2, timeout=10, context=_ctx) as resp2:  # nosec B310 - URL SUPABASE env, https validado
