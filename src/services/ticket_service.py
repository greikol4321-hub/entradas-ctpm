"""Ticket service — comprar, validar, revertir, generar_codigo — ponytail: 1 conexión con FOR UPDATE"""
import secrets, uuid, pathlib
...
def comprar_ticket(nombre, cedula, ubicacion, mesa_numero_raw, telefono, file_bytes, filename, upload_folder):
    ...
    try:
        ...
        cur.execute("INSERT INTO entradas (id, codigo, nombre_completo, cedula, ubicacion, mesa_numero, monto, telefono, comprobante_path) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s)",
                    (eid, codigo, nombre, cedula, ubicacion, mesa_numero, monto, telefono, f"uploads/{fname}"))
        conn.commit()
    ...
    database.log_audit("comprar", eid, {"numero": numero, "codigo": codigo, "ubicacion": ubicacion, "mesa": mesa_numero, "monto": monto})
    database.invalidate_all_cache()
    return {"ok": True, "id": eid, "codigo": codigo, "numero": numero, "monto": monto, "mesa": mesa_numero}
